...
def run(Inst, Reg, Hist = [], Mul = [], point = 0):
    while point < len(Inst):
        ...
        if inst[0] == 'cpy':
            ...
            if inst[1] in Reg.keys():
                ...
                out = inst[1]
            Hist.append( (point, inst, out) )
            point += 1
            break
        else:
            assert False
    return Reg, Hist, Mul, point, out
...
# part 1
# b/c/d are immediately initialized, so we can give them any value to start
a = 1
ans1 = None
while ans1 is None:
    Reg = {'a': a, 'b': 0, 'c': 0, 'd': 0}
    ...
    Out = []
    for _ in range(20):
        Reg, Hist, Mul, point, out = run(Inst, Reg, Hist, Mul, point)
        Out.append(out)
    if Out == [0,1]*10:
        ans1 = a
    a += 1
